...
class TestPastAppts(CustomerAuthTestCase):
	url = reverse("customer/appointments/past")
	...
	def test_past_appts_unauthorized(self):
		response = self.client.get(self.url)
		self.assertEqual(response.status_code, 401)

	def test_past_appts(self):
	...
	def test_past_appts_available(self):
		generate_past_appts(self.customer, 5)
		response = self.client.get(self.url,**self.customer_headers)
		self.assertEqual(response.status_code, 200)
