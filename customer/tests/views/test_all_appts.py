...
from customer.tests.views.generate_customer_appts import generate_customer_appts


class TestAllAppts(CustomerAuthTestCase):
	url = reverse("customer/appointments/all")
	...
	def test_all_appts_unauthorized(self):
		response = self.client.get(self.url)
		self.assertEqual(response.status_code, 401)

	def test_all_appts(self):
	...
	def test_all_appts_available(self):
		generate_customer_appts(self.customer, 5)
		response = self.client.get(self.url,**self.customer_headers)
		self.assertEqual(response.status_code, 200)
