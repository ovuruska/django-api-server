...
def set_employee_working_hours(employee_id, date, start,end, branch_id):
	...
	if branch_id != None:
		created = {
			...
			"week_day": date.weekday(),
			"start": datetime.datetime.fromisoformat(start).replace(tzinfo=None) if start is not None else None,
			"end":datetime.datetime.fromisoformat(end).replace(tzinfo=None) if end is not None else None,
			"branch_id": branch_id
