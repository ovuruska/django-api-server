...
class TestAppointmentEmployeeCreateAPIView(AuthTestCase):
    ...
    def test_employee_create_appointment(self):
        ...
        response = self.client.post('/api/schedule/appointment-employee',data = {
            'customer_name': f"{self.customer_name}",
            'customer_email': f"{self.customer_email}",
            'customer_phone': f"{self.customer_phone}",
            'dog_name': f"{self.dog_name}",
            'dog_breed': f"{self.dog_breed}",
            'products': [self.product.id],
            'services': [self.service.id],
            "start": "2023-01-13T19:15:17",
            "branch_id": f"{self.branch.id}",
            "employee_id": f"{self.employee.id}",
