...
class EmployeeFreeTimesView(generics.GenericAPIView):

	def post(self, request, *args, **kwargs):
		branches = request.data.get("branches",[])
		employees = request.data.get("employees",[])
		date = request.data.get("start","")
		duration = request.data.get("duration",60)

		date = datetime.strptime(date, "%d-%m-%Y")
		available_slots = get_available_appointment_slots(branches, employees, date,service_type="We Wash",duration = duration)
