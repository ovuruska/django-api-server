...
def get_slots(working_interval: IntervalEntity, appointments: [IntervalEntity], minutes=60):
	duration = datetime.timedelta(minutes=minutes)
	available_slots = []
	slots = sorted([(working_interval.start, working_interval.start)] + appointments + [
...
def get_daily_available_slots(working_hours, appointments, duration):

	all_slots = []
	for working_hour in working_hours:
		employee_appointments = [appointment for appointment in appointments if appointment.employee_id == working_hour.employee_id]
		available_slots = get_slots(working_hour, employee_appointments, duration)
		all_slots.extend([
			{
				'start': slot[0],
				'end': slot[1],
				'employee_id': working_hour.employee_id,
				'branch_id': working_hour.branch_id
			}
